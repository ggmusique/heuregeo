...
test.describe("Hiérarchie temporelle du bilan mobile", () => {
  test("1. Le mode mois affiche des cartes semaine, sans doublon de cartes mission", async ({ page }) => {
    const found = await goToBilan(page);
    if (!found) {
      test.skip(true, "Page Bilan non accessible dans cette configuration mobile");
      return;
    }
    ...
    expect(overflow, "Overflow horizontal sur la vue mois du bilan (iPhone)").toBe(false);
  });

  test("2. Le mode année affiche des cartes mois et garde un layout propre sur iPhone", async ({ page }) => {
    const found = await goToBilan(page);
    if (!found) {
      test.skip(true, "Page Bilan non accessible dans cette configuration mobile");
      return;
    ...
    const overflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth);
    expect(overflow, "Overflow horizontal sur la vue année du bilan (iPhone)").toBe(false);
  });
});