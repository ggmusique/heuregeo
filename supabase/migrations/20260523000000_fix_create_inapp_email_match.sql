-- =============================================================================
-- Migration 20260523000000 : create_inapp_invitation – recherche 4 étapes
-- =============================================================================
-- Problème : le match par nom seul échoue quand l'ouvrier a tapé "Michel"
-- mais le profil contient "Michel Martin" (ou inversement).
-- Solution : ajouter une étape intermédiaire de match par email, qui est
...
--   3. lower(nom) = lower(v_patron_name) + NULL  → même nom complet
--   4. INSERT si rien trouvé
-- Pour les étapes 2 et 3 : UPDATE linked_user_id sur la ligne existante.
-- =============================================================================
...
BEGIN
  ...
  IF v_patron_id IS NULL THEN
    -- Récupérer l'email auth du patron
  ...
  -- ── Étape 3 : correspondance par nom complet ──────────────────────────────
  IF v_patron_id IS NULL THEN
    SELECT id INTO v_patron_id
      FROM public.patrons
     WHERE user_id        = v_owner_id
  ...
  -- ── Étape 4 : aucune correspondance → INSERT ──────────────────────────────
  IF v_patron_id IS NULL THEN
