...
export function useBilan({
  ...
  const [bilanContent, setBilanContent] = useState<BilanContent>({
    titre: "",
    totalE: 0,
    totalH: 0,
    filteredData: [],
  ...
  const isGlobalPatronId = (patronId: string | null | undefined): boolean =>
    !patronId || patronId === GLOBAL_PATRON_ID;

  const resolvePatronNom = (patronId: string | null | undefined): string => {
    ...
    async (currentWeek: number, patronId: string | null = null): Promise<number> => {
      ...
      try {
        ...
        const impayePrecedent = bilans.reduce((sum: number, r: Record<string, unknown>) => {
          ...
          const alloue = (allocByWeek as Record<string | number, number>)[r.periode_index as string | number] || 0;
  ...
  const getAcomptesUtilisesAvantPeriode = useCallback(
    async (weekNum: number, patronId: string | null = null): Promise<number> => {
      ...
      try {
        ...
        return (data || []).reduce((sum: number, row: Record<string, unknown>) => sum + (parseFloat(String(row?.amount)) || 0), 0);
      } catch (err) {
  ...
  const genererBilan = useCallback(
    async (patronId: string | null = null, clientId: string | null = null): Promise<boolean | void> => {
      ...
      try {
        ...
        const filtered: Mission[] = getMissionsByPeriod(
          bilanPeriodType,
          bilanPeriodValue,
          runPatronId
        ...
        let groupedData: BilanGroupedRow[] = [];
        if (bilanPeriodType === PERIOD_TYPES.MOIS && filtered.length > 0) {
          const weekMap = new Map<number, { weekNumber: number; totalHeures: number; totalMontant: number; missions: Mission[] }>();
          filtered.forEach((m) => {
        ...
        if (bilanPeriodType === PERIOD_TYPES.ANNEE && filtered.length > 0) {
          const monthMap = new Map<string, { monthKey: string; totalHeures: number; totalMontant: number; missions: Mission[] }>();
          filtered.forEach((m) => {
          ...
          groupedData = Array.from(monthMap.values())
            ...
            .map((month) => {
              ...
              const monthLabel = new Date(Number(year), parseInt(monthNum) - 1)
        ...
        let fraisKm: BilanKmResult = { items: [], totalKm: 0, totalAmount: 0 };
        ...
        if (bilanPeriodType === PERIOD_TYPES.SEMAINE) {
          const safeNumber = (v: unknown): number => (Number.isFinite(Number(v)) ? Number(v) : 0);
  ...
  const rebuildBilans = useCallback(
    async (patronId: string | null, startWeek: number, endWeek: number): Promise<RebuildResult> => {
      const pId = effectivePatronId(patronId);
      try {
        const user = await getCurrentUserOrNull();
        ...
        return { success: false, message: (err as Error)?.message || "Erreur rebuild" };
      }
    },
    [getMissionsByPeriod, getFraisByWeek, getTotalFrais, getImpayePrecedent]
  );

  const repairBilansDB = useCallback(async (patronId: string | null): Promise<RepairResult> => {
    const pId = effectivePatronId(patronId);
    try {
      const user = await getCurrentUserOrNull();
      ...
      return { success: false, message: (err as Error)?.message || "Erreur inconnue", fixed: 0, skipped: 0 };
    }
  }, []);
