...
export default function App({ user }) {
  ...
  const handleFraisDelete = async (frais) => {
    const confirmed = await showConfirm({
      ...
      message: `Voulez-vous vraiment supprimer "${frais.description}" ?`,
      confirmText: "Supprimer",
      cancelText: "Annuler",
      type: "danger",
    });
  ...
  const handleMarquerCommePaye = async () => {
    const confirmed = await showConfirm({
      ...
      message: `Voulez-vous marquer ce bilan comme payé ?`,
  ...
  const chargerHistorique = async (patronId = null) => {
    if (typeof bilan.fetchHistoriqueBilans !== "function") {
      triggerAlert("⚠️ Historique non branché");
      return;
    }
    try {
      ...
      <main className="relative px-5 -mt-10 pb-32 z-10">
        ...
        {activeTab === "historique" && (
          <HistoriqueTab
            ...
            historiquePatronId={historiquePatronId}
            historiqueTab={historiqueTab}
            ...
            onPatronFilterChange={(patronId) => {
              setHistoriquePatronId(patronId);
              chargerHistorique(patronId);
            }}
      ...
      <nav className="fixed bottom-6 left-6 right-6 z-[100]">
        <div className="bg-[#020818]/90 backdrop-blur-3xl border border-yellow-600/20 p-2 rounded-[35px] shadow-2xl flex gap-1">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-[28px] font-black uppercase text-[9px] tracking-widest flex flex-col items-center justify-center gap-0.5 ${
              activeTab === "compte"
