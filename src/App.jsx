...
export default function App({ user }) {
  ...
  } = useKmDomicile({ profile, saveProfile, lieux, getMissionsByWeek });
  ...
  const missionForm = useMissionForm({
    createMission,
    updateMission,
    deleteMission,
    missions,
  ...
  const lieuModal = useLieuModal({
    ...
    triggerAlert,
    showConfirm,
    onLieuCreated: missionForm.onLieuCreated,
  });

  const historiqueHook = useHistorique({
    fetchHistoriqueBilans: bilan.fetchHistoriqueBilans,
    triggerAlert,
  });

  const acompteModal = useAcompteModal({
    createAcompte,
    fetchAcomptes,
    setLoading,
    triggerAlert,
    bilanPatronId,
    chargerHistorique: historiqueHook.chargerHistorique,
    bilan,
  });

  const fraisModal = useFraisModal({
    createFrais,
    updateFrais,
    deleteFrais,
    setLoading,
  ...
  const patronModal = usePatronModal({
    createPatron,
    updatePatron,
    deletePatron,
    setLoading,
  ...
  const agendaModal = useAgendaModal({
    createEvent: agendaHook.createEvent,
    updateEvent: agendaHook.updateEvent,
    deleteEvent: agendaHook.deleteEvent,
    triggerAlert,
  });
  ...
  const agendaWorkedDays = useMemo(() => {
    const s = new Set();
    missions.forEach((m) => {
      if (m.date_iso) s.add(m.date_iso);
    });
    return s;
  }, [missions]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(
        ...
        showMissionRateEditor ? "true" : "false"
      );
    }
  }, [showMissionRateEditor]);
  ...
  const proNavItems = [
    ...
    ...(canAgenda
      ...
      : []),
    ...(canDashboard
      ? [{ key: "dashboard", label: "Dashboard", icon: "📈", activeClass: "from-yellow-600 to-amber-700" }]
      : []),
  ...
  const isLoading =
    ...
    lieuxLoading ||
    gpsLoading ||
    historiqueHook.loadingHistorique ||
    agendaHook.loading;
      ...
      <ThemeContext.Provider value={themeProvider}>
        <div
          className={
            ...
            (isDark ? "dark bg-[#020818] text-white" : "light " + themeConfig.bg)
          }
        >
          <div className="fixed inset-0 pointer-events-none">
            {themeConfig.overlay && (
              <div className={"absolute inset-0 " + themeConfig.overlay} />
            )}
              ...
              <SaisieTab
                ...
                onLieuChange={(lieuId) => {
                  ...
                  if (missionForm.editingMissionId) {
                    ...
                    missionForm.setEditingMissionData((prev) => ({
                      ...(prev || {}),
                      lieu_id: lieuId,
                ...
                onShowLieuModal={() => {
                  ...
                  lieuModal.setShowLieuModal(true);
                }}
                onShowClientModal={() => {
                  clientModal.resetClientForm();
                  clientModal.setShowClientModal(true);
                }}
                onShowPatronModal={() => {
                  patronModal.resetPatronForm();
                  patronModal.setShowPatronModal(true);
                }}
            ...
            {activeTab === "suivi" && (
              <SuiviTab
                ...
                historiqueProps={{
                  ...
                  missions,
                  listeAcomptes,
                  onPatronFilterChange: (patronId) => {
                    historiqueHook.setHistoriquePatronId(patronId);
                    historiqueHook.chargerHistorique(patronId);
                  },
