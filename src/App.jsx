...
// ============================================================
// ✅ REMPLACE TOUT LE BLOC activeTab === "historique" dans App.jsx
// ============================================================

        ) : /* ========= ONGLET HISTORIQUE + DASHBOARD ========= */
        activeTab === "historique" ? (
          <div className="animate-in fade-in duration-500 space-y-4 pb-4">

            {/* ── SÉLECTEUR PATRON ── */}
            <div className={`p-4 rounded-[25px] border ${
              darkMode ? "bg-white/5 border-white/10" : "bg-white border-slate-200"
            } backdrop-blur-md`}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setHistoriquePatronId(val);
                  chargerHistorique(val);
                }}
            ...
            {/* ── TABLEAU DE BORD ── */}
            {(historique.all?.length > 0 || historique.impayes?.length > 0 || historique.payes?.length > 0) && (() => {
              // Calculs dashboard
              const currentMonth = new Date().toISOString().substring(0, 7);
              const currentWeek = getWeekNumber(new Date());

              // CA ce mois = missions du mois en cours
              const missionsMois = missions.filter(m => {
                const patronOk = !historiquePatronId || m.patron_id === historiquePatronId;
                return patronOk && m.date_iso?.startsWith(currentMonth);
              });
              const caMois = missionsMois.reduce((s, m) => s + (m.montant || 0), 0);

              // CA total
              const missionsTotal = missions.filter(m =>
                !historiquePatronId || m.patron_id === historiquePatronId
              );
              const caTotal = missionsTotal.reduce((s, m) => s + (m.montant || 0), 0);

              // Total impayés
              const totalImpayes = historique.impayes.reduce((s, r) =>
                s + (r.reste_a_percevoir || 0), 0
              );

              // Dernier acompte
              const acomptesFiltres = listeAcomptes.filter(a =>
                !historiquePatronId || a.patron_id === historiquePatronId
              );
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className={`p-4 rounded-[20px] border ${
                      darkMode ? "bg-indigo-900/30 border-indigo-500/20" : "bg-indigo-50 border-indigo-200"
                    }`}>
                    ...
                    <div className={`p-4 rounded-[20px] border ${
                      darkMode ? "bg-purple-900/30 border-purple-500/20" : "bg-purple-50 border-purple-200"
                    }`}>
                      <p className="text-[9px] font-black uppercase opacity-50 tracking-widest mb-1">
                        CA total
                      </p>
                  ...
                  {/* Total impayés */}
                  <div className={`p-4 rounded-[20px] border flex items-center justify-between ${
                    totalImpayes > 0
                  ...
                  {/* Semaines payées / impayées + dernier acompte */}
                  <div className="grid grid-cols-2 gap-3">
                    <div className={`p-4 rounded-[20px] border ${
                      darkMode ? "bg-green-900/20 border-green-500/20" : "bg-green-50 border-green-200"
                    }`}>
                      <p className="text-[9px] font-black uppercase opacity-50 tracking-widest mb-1">
                        Semaines payées
                      </p>
                    ...
                    <div className={`p-4 rounded-[20px] border ${
                      darkMode ? "bg-orange-900/20 border-orange-500/20" : "bg-orange-50 border-orange-200"
                    }`}>
                      <p className="text-[9px] font-black uppercase opacity-50 tracking-widest mb-1">
                        Semaines impayées
                      </p>
                  ...
                  {dernierAcompte && (
                    <div className={`p-4 rounded-[20px] border flex items-center justify-between ${
                      darkMode ? "bg-cyan-900/20 border-cyan-500/20" : "bg-cyan-50 border-cyan-200"
                    }`}>
                      <div>
                        <p className="text-[9px] font-black uppercase opacity-50 tracking-widest mb-1">
                          Dernier acompte reçu
            ...
            {/* ── BOUTON CHARGER (si pas encore chargé) ── */}
                      ...
                      <div className="space-y-2">
                        {historique.impayes.map((row) => (
                          <div
                            ...
                            className={`flex items-center justify-between p-4 rounded-2xl ${
                              darkMode ? "bg-black/30 border border-white/10" : "bg-white border border-orange-100"
                            }`}
                          >
                            <div>
                        ...
                        <div className="pt-2 border-t border-orange-500/20 flex justify-between items-center px-2">
                          ...
                          <p className="text-xl font-black text-orange-400">
                            {formatEuro(historique.impayes.reduce((s, r) => s + (r.reste_a_percevoir || 0), 0))}
                          </p>
                        </div>
                      </div>
                    )}
                ...
                {historiqueTab === "payes" && (
                  <div className={`p-4 rounded-[25px] border ${
                    ...
                    {historique.payes.length === 0 ? (
                      <div className="text-center py-6">
                        <p className="text-3xl mb-2">📭</p>
                        <p className="text-sm opacity-60 font-bold">Aucun paiement enregistré.</p>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {historique.payes.map((row) => (
                          <div
                            key={row.id}
                            className={`flex items-center justify-between p-4 rounded-2xl ${
                              darkMode ? "bg-black/30 border border-white/10" : "bg-white border border-green-100"
                            }`}
                          >
                            <div>
                              <p className="font-black uppercase text-sm">
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-[9px] opacity-40">
                                {row.date_paiement ? formatDateFR(row.date_paiement) : ""}
                              </p>
                            </div>
                          </div>
                        ))}
