 ...
 */
const generateDocRef = (periodType: string, periodValue: string): string => {
...
// EN-TÊTE
// ─────────────────────────────────────────────
const drawHeader = (doc: any, bilanContent: any, periodType: string, periodValue: string): number => {
  try {
    const pageW = 210;
...
// PROFIL UTILISATEUR
// ─────────────────────────────────────────────
const drawProfileSection = (doc: any, profile: any, startY: number): number => {
...
// CARTES KPI
// ─────────────────────────────────────────────
const drawMetrics = (doc: any, bilanContent: any, periodType: string, startY: number): number => {
  try {
    const getFittedFontSize = (text: string, maxWidth: number, initialSize: number, minSize: number = 7): number => {
    ...
    let metrics: Array<{ label: string; value: string; accent: number[] }> = [
...
// ─────────────────────────────────────────────
const drawTableHeader = (doc: any, startX: number, y: number, headers: string[], colWidths: number[], bgColor: number[]): number => {
...
// ─────────────────────────────────────────────
const drawMissionsTable = (doc: any, bilanContent: any, startY: number, labels: Record<string, string> = {}): number => {
  ...
  try {
    ...
    bilanContent.filteredData.forEach((m: any, idx: number) => {
      ...
      const lieuObj = lieux.find((l: any) => l.id === m.lieu_id)
        || (m.lieu ? lieux.find((l: any) => l.nom?.toLowerCase().trim() === m.lieu?.toLowerCase().trim()) : null);
    ...
    // Total missions
    const totH = bilanContent.filteredData.reduce((s: number, m: any) => s + (m.duree || 0), 0);
    const totE = bilanContent.filteredData.reduce((s: number, m: any) => s + (m.montant || 0), 0);
...
// TABLEAU FRAIS
// ─────────────────────────────────────────────
const drawFraisTable = (doc: any, bilanContent: any, startY: number): number => {
  ...
  try {
    ...
    bilanContent.fraisDivers.forEach((f: any, idx: number) => {
      if (y > 268) { doc.addPage(); y = 20; }
      if (idx % 2 === 0) {
        doc.setFillColor(...COLORS.light);
        doc.rect(startX, y - 1, tableW, 7.5, "F");
...
// FRAIS KILOMÉTRIQUES
// ─────────────────────────────────────────────
const drawKmFeesTable = (doc: any, bilanContent: any, startY: number, labels: Record<string, string> = {}): number => {
  const km = bilanContent.fraisKilometriques;
  if (!km?.totalAmount || km.totalAmount <= 0) return startY;
  const validItems = (km.items || []).filter((i: any) => i.amount !== null && i.amount > 0);
...
// SECTION ACOMPTES
// ─────────────────────────────────────────────
const drawAcomptesSection = (doc: any, bilanContent: any, startY: number): number => {
  const hasAcomptes =
...
// ─────────────────────────────────────────────
const drawWeeklySummary = (doc: any, bilanContent: any, startY: number): number => {
  ...
  try {
    ...
    bilanContent.groupedData.forEach((g: any, idx: number) => {
      if (y > 268) { doc.addPage(); y = 20; }
      if (idx % 2 === 0) {
        doc.setFillColor(...COLORS.light);
        doc.rect(startX, y - 1, tableW, 7.5, "F");
...
// ─────────────────────────────────────────────
const drawMonthlySummary = (doc: any, bilanContent: any, startY: number): number => {
  if (!bilanContent.groupedData?.length) return startY;
  try {
    let y = startY;
    doc.setFontSize(11);
