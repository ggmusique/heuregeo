 ...
 * Note: L'export PDF est géré par exportPDF_Pro.ts
 */
...
export const exportToExcel = (
  ...
  if (periodType === "semaine" && Array.isArray(fraisDivers)) {
    totalFrais = fraisDivers.reduce(
      (sum: number, f: any) => sum + (parseFloat(f?.montant) || 0),
      0
    );
  }
...
export const exportToCSV = (
  ...
  if (
    ...
    const fraisRows = bilanContent.fraisDivers.map((f: any) => {
      if (!f) return [];
      return [
    ...
    csvContent += fraisRows.map((row: any[]) => row.join(";")).join("\n");
...
// ========== EXPORT PDF (LEGACY) ==========
// Note: Utilisez exportPDF_Pro.ts pour l'export PDF
// Cette fonction est conservée pour compatibilité si nécessaire

/**
 * @deprecated Utilisez exportToPDFPro de exportPDF_Pro.ts à la place
 */
