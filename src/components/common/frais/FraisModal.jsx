    ...
    <div className={`fixed inset-0 z-[500] flex items-center justify-center p-6 ${isDark ? "bg-[#050510]/90" : "bg-black/40"} backdrop-blur-md`}>
      <div
        className={`w-full max-w-sm p-8 rounded-[40px] border-2 ${
          isDark ? "bg-[#121420] border-white/10" : "bg-white border-slate-200"
        } backdrop-blur-xl`}
      >
        ...
        <input
          ...
          className={`w-full p-5 rounded-2xl mb-2 font-bold outline-none border backdrop-blur-md transition-all ${
            isDark ? "bg-black/20 text-white border-white/5" : "bg-slate-50 text-slate-900 border-slate-200"
        ...
        <DateSelector dateMission={date} setDateMission={setDate} isIOS={isIOS} />
        {helper && (
          <p className={`mt-4 text-[10px] ${isDark ? "text-white/50" : "text-slate-400"}`}>
            ⚠️ {helper}
          </p>
        )}
        <div className="flex gap-3 mt-6">
          <button
            onClick={onCancel}
            className={`flex-1 py-4 rounded-2xl text-[10px] font-black backdrop-blur-md ${isDark ? "bg-white/5" : "bg-slate-100 text-slate-600"}`}
          >
            ANNULER
          </button>
          <button
            ...
            className={`flex-1 py-4 rounded-2xl text-[10px] font-black text-white backdrop-blur-md transition-all ${
              canSubmit ? "bg-amber-600 active:scale-95" : "bg-gray-600/40 opacity-60 cursor-not-allowed"
            }`}
