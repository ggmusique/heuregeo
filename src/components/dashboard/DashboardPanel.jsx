...
export function DashboardPanel({
  ...
  const computeKmForMissions = useCallback(
    (missionsList) => {
      ...
      if (
        !Number.isFinite(effectiveDomicile?.lat) ||
        !Number.isFinite(effectiveDomicile?.lng)
      ) {
        return { totalKm: 0, totalAmount: 0 };
      }
      ...
      missionsList.forEach((m) => {
        ...
        const lieuByName =
          !lieuById && m.lieu
            ? lieux.find(
                (l) =>
                  l.nom?.toLowerCase().trim() ===
                  m.lieu?.toLowerCase().trim()
              )
            : null;
  ...
  const kmThisWeek = useMemo(
    () => computeKmForMissions(missionsThisWeek),
    [computeKmForMissions, missionsThisWeek]
  );
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ca: {
        value: thisW.ca,
        delta: lastW.ca
          ? Math.round(((thisW.ca - lastW.ca) / lastW.ca) * 100)
          : null,
      },
      hours: {
        value: thisW.hours,
      ...
      uniqueClients: new Set(
        missionsThisWeek.map((m) => m.client || m.client_id)
      ).size,
    };
  ...
  const bilanSemaine = useMemo(() => {
    ...
    const fraisWeek = filterWeek(fraisDivers, "date_frais").reduce(
      (s, f) => s + (parseFloat(f.montant) || 0),
      0
    );
    const acomptesWeek = filterWeek(listeAcomptes, "date_acompte").reduce(
      (s, a) => s + (parseFloat(a.montant) || 0),
      0
    );
    ...
    return {
      ...
      resteApercevoir: Math.max(
        0,
        kpis.ca.value + fraisWeek + kmThisWeek.totalAmount - acomptesWeek
      ),
    };
  }, [
    kpis.ca.value,
  ...
  const previousWeeks = useMemo(() => {
    return Array.from({ length: 5 }, (_, i) => {
      ...
      const weekMissions = filteredMissions.filter((m) => {
        ...
        const d = new Date(m.date_iso + "T12:00:00");
        return (
          getWeekNumber(d) === weekNum && getISOWeekYear(d) === currentYear
        );
      });
  ...
  const topClients = useMemo(() => {
    ...
    filteredMissions.forEach((m) => {
      ...
      const name =
        m.client || clients.find((c) => c.id === m.client_id)?.nom || "Client";
      ...
      >
        ...
        >
              ...
              >
                <span
                  style={{
                    ...
                    height: "10px",
                    borderRadius: "3px",
                    background: chartColors.indigo.primary,
              ...
              >
                <span
                  style={{
                    ...
                    height: "10px",
                    borderRadius: "3px",
                    background: chartColors.amber.primary,
                    display: "inline-block",
                  }}
                  aria-hidden="true"
                />
          ...
          <div style={{ position: "relative", height: "180px", width: "100%" }}>
            ...
            <canvas
              ref={chartRef}
              role="img"
              aria-label="Graphique du chiffre d'affaires sur 3 mois"
            />
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              {kmThisWeek.totalAmount > 0
                ? formatEuro(kmThisWeek.totalAmount)
                : "—"}
            </span>
            ...
            >
              {kmEnabled
                ? "Calcul basé sur les missions de la semaine en cours."
                : "Le kilométrage n'est pas activé dans les paramètres."}
            </div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {formatEuro(bilanSemaine.caWeek)}
            </span>
          </div>
          ...
          {bilanSemaine.fraisWeek > 0 && (
            ...
            >
              ...
              >
                +{formatEuro(bilanSemaine.fraisWeek)}
              </span>
            </div>
          )}
          ...
          {kmEnabled && bilanSemaine.kmWeek > 0 && (
            ...
            >
              ...
              >
                +{formatEuro(bilanSemaine.kmWeek)}
              </span>
            </div>
          )}
          ...
          {bilanSemaine.acomptesWeek > 0 && (
            ...
            >
              ...
              >
                -{formatEuro(bilanSemaine.acomptesWeek)}
              </span>
            </div>
          )}
        ...
        >
          ...
          {previousWeeks.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              {previousWeeks.map((w, idx) => {
                ...
                const hasDetails =
                  w.frais > 0 || (kmEnabled && w.km > 0) || w.acomptes > 0;
                  ...
                  >
                    <div
                      style={{
                        ...
                        padding: "9px 12px",
                        background:
                          idx === 0 ? "rgba(212,175,55,0.04)" : "transparent",
                      }}
                    >
                      <div
                        style={{
          ...
          >
            ...
            >
              {formatEuro(
                filteredMissions.reduce((s, m) => s + (m.montant || 0), 0)
              )}
            </span>
          </div>
        </div>
