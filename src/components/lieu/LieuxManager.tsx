        ...
        <div className={`p-4 rounded-2xl border ${darkMode ? "border-blue-500/30 bg-blue-500/5" : "border-blue-300 bg-blue-50"}`}>
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-sm text-white/60">{lieuxSansCoords.length} lieu{lieuxSansCoords.length > 1 ? "x" : ""} sans coordonnées GPS</p>
            </div>
            <button onClick={handleBackfillCoords} disabled={backfillRunning} className="px-4 py-2 rounded-xl bg-blue-600/20 border border-blue-500/40 text-blue-300 text-[10px] font-black uppercase tracking-wider hover:bg-blue-600/30 disabled:opacity-50 transition-all">
              {backfillRunning ? "En cours..." : "🗺️ Compléter les coordonnées GPS"}
            </button>
          </div>
          {backfillProgress && <p className="mt-2 text-sm text-white/60">{backfillProgress.done} / {backfillProgress.total} lieux traités</p>}
          {backfillResult && (
            <div className="mt-2 space-y-1">
              ...
              {backfillResult.errors.map((e, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <button onClick={() => onLieuEdit(lieux.find((l) => l.id === e.id))} className="text-[10px] font-black uppercase text-purple-300 hover:text-purple-100 transition-all">Modifier</button>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="space-y-3">
        {sortedLieux.length === 0 ? (
          <div className="text-center py-12">
            ...
            {!searchTerm && <button onClick={onAdd} className="mt-4 px-6 py-3 bg-purple-600 hover:bg-purple-700 rounded-2xl font-black text-white text-[11px] uppercase transition-all">Créer le premier lieu</button>}
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1 mb-3">
                      {lieu?.adresse_complete && <div className="text-xs text-white/60 flex items-start gap-2"><span className="shrink-0">📍</span><span className="line-clamp-2">{lieu.adresse_complete}</span></div>}
                      {hasGps && <div className="text-xs text-white/60 flex items-center gap-2"><span>🌐</span><span>GPS: {lieu.latitude.toFixed(6)}, {lieu.longitude.toFixed(6)}</span></div>}
                      {lieu?.notes && <div className="text-xs text-white/60 flex items-start gap-2"><span className="shrink-0">📝</span><span className="line-clamp-2">{lieu.notes}</span></div>}
                      {needsRegeocode && onRegeocoderLieu && (
                        <div className="mt-1">
                          <button onClick={() => handleRegeocoderLieu(lieu)} disabled={!!regeocodingId} className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all disabled:opacity-50 ${suspect ? "bg-amber-600/20 border border-amber-500/40 text-amber-300 hover:bg-amber-600/30" : "bg-blue-600/20 border border-blue-500/40 text-blue-300 hover:bg-blue-600/30"}`}>{isRegeocoding ? "🔄 Recherche…" : "🗺️ Re-géocoder"}</button>
                  ...
                  {allowActions && (
                    <div className="flex gap-2">
                      <button onClick={() => onEdit(lieu)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all active:scale-90 ${darkMode ? "bg-blue-600/20 text-blue-400 border border-blue-500/30 hover:bg-blue-600/30" : "bg-blue-100 text-blue-600 border border-blue-300 hover:bg-blue-200"}`} title="Modifier">✏️</button>
                      <button onClick={() => onDelete(lieu)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all active:scale-90 ${darkMode ? "bg-red-600/20 text-red-400 border border-red-500/30 hover:bg-red-600/30" : "bg-red-100 text-red-600 border border-red-300 hover:bg-red-200"}`} title="Supprimer">🗑️</button>
                    </div>
                  )}
                </div>
              </div>
