...
export function PatronModal({ show, editMode = false, initialData = null, onSubmit, onCancel, loading = false, darkMode = true }: Props) {
  const L = useLabels();
  ...
  const COULEURS_PRESET = [
    { nom: "Violet", value: "#8b5cf6" }, { nom: "Bleu", value: "#3b82f6" }, { nom: "Vert", value: "#10b981" }, { nom: "Rouge", value: "#ef4444" },
    { nom: "Orange", value: "#f97316" }, { nom: "Rose", value: "#ec4899" }, { nom: "Jaune", value: "#eab308" }, { nom: "Cyan", value: "#06b6d4" },
  ];
  ...
  const handleSubmit = () => {
    ...
    onSubmit?.({
      ...
      adresse: adresse.trim() || null, code_postal: codePostal.trim() || null, ville: ville.trim() || null,
      telephone: telephone.trim() || null, email: email.trim() || null, siret: siret.trim() || null,
    });
  };
      ...
      <div className={`relative w-full max-w-md rounded-[35px] p-8 shadow-2xl animate-in slide-in-from-bottom-4 duration-300 max-h-[calc(100vh-8rem)] overflow-y-auto ${darkMode ? "bg-gradient-to-br from-indigo-900/95 to-purple-900/95 text-white" : "bg-white text-slate-900"}`}>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <input type="text" inputMode="decimal" value={tauxHoraire} onChange={(e) => setTauxHoraire(e.target.value)} placeholder="Ex: 15.50" className={inputCls(!tauxIsValid ? "border-red-500/60" : "")} disabled={loading} />
            {!tauxIsValid ? <p className="text-[9px] text-red-300 mt-1 px-1 font-bold uppercase tracking-wider">Taux invalide (&gt;= 0)</p> : <p className="text-[9px] opacity-50 mt-1 px-1">Si renseigné, sera utilisé pour calculer automatiquement le montant</p>}
          </div>
          <div>
        ...
        <div className={`mt-6 p-4 rounded-2xl ${darkMode ? "bg-black/20 border border-white/10" : "bg-slate-100 border border-slate-200"}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              {tauxParsed !== null && !Number.isNaN(tauxParsed) && <div className="text-[10px] opacity-70 font-black uppercase">{tauxParsed.toFixed(2)} €/h</div>}
            </div>
          </div>
        </div>
