...
export function StatsCharts({ missions, patrons, effectivePatronId }) {
  ...
  const filtered = useMemo(
    () =>
      effectivePatronId
        ? missions.filter((m) => m.patron_id === effectivePatronId)
        : missions,
  ...
  const { monthlyCA, monthlyHours } = useMemo(() => {
    ...
    const monthlyCA    = keys.map((k) => ({ label: monthLabel(k), value: byMonth[k].ca,    tooltip: `${k} — ${formatEuro(byMonth[k].ca)}` }));
    const monthlyHours = keys.map((k) => ({ label: monthLabel(k), value: byMonth[k].hours, tooltip: `${k} — ${byMonth[k].hours.toFixed(1)} h` }));
  ...
  const topPatrons = useMemo(() => {
    const by = {};
    filtered.forEach((m) => {
      const pNom = patrons.find((p) => p.id === m.patron_id)?.nom || "?";
      by[pNom] = (by[pNom] || 0) + (m.montant || 0);
    });
    return Object.entries(by)
      .map(([label, value]) => ({ label, value }))
      .sort((a, b) => b.value - a.value)
  ...
  if (filtered.length === 0) {
    return (
      <div className="text-center py-4 opacity-50 text-xs font-bold">
