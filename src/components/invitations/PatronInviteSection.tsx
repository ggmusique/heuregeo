...
/**
 * Section d'invitations in-app pour les PATRONS (employeurs).
 * Intégrée dans PatronView → onglet Équipe.
 *
 * - Bloc 0: Ouvriers connectés (connexions acceptées) avec toggles d'accès
 * - Bloc 1: Mon code unique (à partager avec un ouvrier)
 * - Bloc 2: Rechercher un ouvrier par code → envoyer une demande
 * - Bloc 3: Invitations reçues (ouvrier → moi) → accepter / refuser
 * - Bloc 4: Invitations envoyées en attente → annuler
 */
export function PatronInviteSection({ onAccessChange }: { onAccessChange?: () => void }) {
  const {
    pendingSent,
    pendingReceived,
    loading,
  ...
  // ── Envoyer une invitation vers un ouvrier ───────────────────────────────

  const handleSend = async () => {
    if (!searchResult) return;
    setSendError(null);
    setSendLoading(true);
    // Le patron initie la demande → initiated_by = 'patron'
    ...
    <div className="space-y-5 p-4">
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-offset)] p-4 space-y-2">
        ...
        <p className="text-xs text-[var(--color-text-muted)]">
          Partage ce code avec un ouvrier pour qu'il puisse t'inviter depuis son application.
        </p>
      ...
      {/* ── Bloc 2: Demander l'accès à un ouvrier ── */}
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-offset)] p-4 space-y-3">
        ...
        <p className="text-xs text-[var(--color-text-muted)]">
          Entre le code de l'ouvrier pour lui envoyer une demande de connexion.
        </p>
      ...
      {/* ── Bloc 3: Invitations reçues (ouvrier → moi) ── */}
      ...
      {/* ── Bloc 4: Invitations envoyées en attente ── */}
      ...
      {pendingReceived.length === 0 && pendingSent.length === 0 && (
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-offset)] px-4 py-8 text-center">
          <p className="text-sm text-[var(--color-text-muted)]">Aucune connexion en attente</p>
        </div>
      )}
    </div>
  );
...
// ─── Carte invitation (patron) ────────────────────────────────────────────────

interface PatronInvCardProps {
  inv: InAppInvitation;
...
function PatronInvCard({ inv, direction, actionLoading, onAccept, onRefuse, onCancel }: PatronInvCardProps) {
  const name = inv.other_name ?? (direction === "received" ? "Ouvrier" : "Ouvrier");
    ...
    <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3.5 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-[var(--color-text-muted)]">
          {direction === "received" ? "Demande reçue" : "Demande envoyée"} · {date}
        </p>
      </div>
      {direction === "received" ? (
